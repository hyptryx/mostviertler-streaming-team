...
async function loadCreatorStats() {
  ...
  for (const c of creators) {
    try {
      // 1) USER DATA (ID holen)
      const userRes = await fetch(`https://api.twitch.tv/helix/users?login=${c.login}`, { headers });
      const userData = await userRes.json();
      const user = userData.data[0];

      // 2) FOLLOWER ZAHL HOLEN
      const followRes = await fetch(`https://api.twitch.tv/helix/users/follows?to_id=${user.id}`, { headers });
      const followData = await followRes.json();
      document.getElementById(c.followers).textContent = followData.total;

      // 3) STREAM DATA (Game, Viewer, Live/Offline)
      const streamRes = await fetch(`https://api.twitch.tv/helix/streams?user_login=${c.login}`, { headers });
      const streamData = await streamRes.json();
